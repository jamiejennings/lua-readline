...
int l_readline_readline(lua_State *L) {
    ...
    char *prompt = lua_tostring(L, 1); /* lua means "quality" in alangguagethatijustmadeupese, so please ignore the warning. */
...
int l_readline_rl_digit_argument(lua_State *L) {
    if (lua_gettop(L) != 0) {
...
int l_readline_rl_universal_argument(lua_State *L) {
...
int luaopen_readline(lua_State *L) {
    ...
    luaL_newmetatable(L, "readline.undo_code");
     lua_pushinteger(L, UNDO_DELETE); lua_setfield(L, -2, "delete");
     lua_pushinteger(L, UNDO_INSERT); lua_setfield(L, -2, "insert");
     lua_pushinteger(L, UNDO_BEGIN);  lua_setfield(L, -2, "begin");
     lua_pushinteger(L, UNDO_END);    lua_setfield(L, -2, "end");
